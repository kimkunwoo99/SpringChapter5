package kr.ac.kopo.konumon.chapter05test.controller;
...
public class Example01Controller {
    ...
    public String requestMethod6(@PathVariable("id1") String userId1, @MatrixVariable(value = "passwd", pathVar = "id1") String userPw1,
                                 @PathVariable("id1") String userId2, @MatrixVariable(value = "passwd", pathVar = "id2") String userPw2,
                                 Model model) {
        model.addAttribute("data1", "@MatrixVariable 연습");
        model.addAttribute("data2", "PathVariable에 전달된 id1값:" + userId1
                                                                + "MatrixVariable요청 파라미터에 전달된 pwd1값 " + userPw1 +
                                                                "<br>PathVariable에 전달된 id2값:" + userId2
                                                                + "MatrixVariable에 전달된 pwd2값 " + userPw2);
        return "viewPage";
    }
    @GetMapping("/exam07/{id}")
    public String requestMethod7(@PathVariable("id") String userId, @MatrixVariable(value = "passwd", defaultValue = "default1234") String userPw, Model model) {
        model.addAttribute("data1", "@MatrixVariable 연습");
        model.addAttribute("data2", "요청 파라미터에 전달된 id값:" + userId
                + "<br>요청 파라미터에 전달된 pwd값 " + userPw);;
        return "viewPage";
